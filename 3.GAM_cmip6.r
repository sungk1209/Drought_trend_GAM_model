...
gam_fit <- gam(precip ~ model + ti(year, bs = "cr", k = 7),
    data=prcp_pos, family = Gamma)

start_time <- Sys.time()
gam_fit <- gam(list(
  precip ~ model + s(month, by = model, bs = "cc", k = 6) + 
    te(year, month, bs = c("cr","cc"), k = c(10,6)), 
...
tmonth <- 7
p <- ggplot(data=prcp_df %>% filter(month == 4), aes(x=year, group=model,colour = model)) +
	geom_line(aes(y=precip)) +
	geom_ribbon(data = modeled_df%>%filter(month == 4), aes(ymin= lowp,
                                     ymax= upp), alpha = 0.25, fill = "black") +
	geom_ribbon(data = modeled_df%>%filter(month == 4), aes(ymin= lowcrit, ymax= upcrit), fill = "black", 
              alpha = 0.5) +
	geom_ribbon(data = modeled_df%>%filter(month == 4), aes(ymin= lower,
                                                          ymax= upper), alpha = 0.25, fill = "black") +
	geom_ribbon(data = modeled_df%>%filter(month == 4), aes(ymin= lowcrit, ymax= upcrit), fill = "black", 
              alpha = 0.5) +
  
	geom_line(data = modeled_df%>%filter(month(date) == 4), aes(y=modGI)) +
   # facet_wrap(~month) +
	labs(title = paste0("FMA"," modeled"),
       subtitle = "",
...
ggsave(filename = paste0(output_path,filename,"3.png"),plot = p, width =12.5, height = 8, dpi = 300)
...
p <- ggplot(data=prcp_pos %>%filter(year %in%  tyear), aes(x= date, group=model,colour = model)) +
  ...
  geom_line(data = modeled_df%>% filter(year %in%  tyear), aes(y=modGI)) +
